...
//class Wolf; // Foward Declaration 
using namespace std;

Rabbit::Rabbit(int fLevel, int mRate, int age, bool moved, bool action, int chance) {
...
// Added Squareyard
void Rabbit::move(int currX, int currY, int maxX, int maxY, SquareYard**& grid) {
	...
	if (valid_check == true) {
		...
		grid[mx][my].SetInnerSquare(sf::Color::Blue);
		grid[mx][my].SetOuterSquare(sf::Color::Blue);
...
// Subtract 3 from food level each day
void Rabbit::changeFoodLevel() {

	this->foodLevel -= this->metabolism;

}

void Rabbit::setMovement(bool change) {
	this->moved = change;
}

void Rabbit::setAction(bool change) {
	this->action = change;
}

void Rabbit::setHunger(bool) {
	// Do Nothing 
}

bool Rabbit::checkToKillCreature(int x, int y, SquareYard**& grid, int day) {
	
	
	// Rabbit dies if food level is 0 or reaches 25 days. ADD CONDITION that is is eaten by a wolf 
	if (grid[x][y].animal->getFoodLevel() <= 0 || day == 25) {
...
bool Rabbit::actionMove(int currX, int currY, int maxX, int maxY, SquareYard **& grid) {
	
	// Will Determine if the Rabbit invalid a wold
	int i = currX;
	int j = currY;
	...
	// Checks if a wolf exists on top
	if ((j - 1 >= 0 && j - 1 < maxY) && (i >= 0 && i < maxX) && grid[i][j - 1].animal != NULL) {

		Creature* up = grid[i][j - 1].animal;

		// Checks to see if its a type of wolf 
		if (dynamic_cast<Wolf *>(up) != 0 && grid[i][j].animal->getFoodLevel() > 30) {

			// Move 2 Left 
			if ((j >= 0 && j < maxY) && (i - 2 >= 0 && i - 2 < maxX) && grid[i - 2][j].animal == NULL) {
			...
			else if ((j + 2 >= 0 && j + 2 < maxY) && (i >= 0 && i < maxX) && grid[i][j + 2].animal == NULL) {
			...
			else if ((j >= 0 && j < maxY) && (i + 2 >= 0 && i + 2 < maxX) && grid[i + 2][j].animal == NULL) {
	...
	if ((j >= 0 && j < maxY) && (i + 1 >= 0 && i + 1 < maxX) && grid[i + 1][j].animal != NULL && pathFound != true) {
		...
		// Checks to see if its a type of wolf 
		if (dynamic_cast<Wolf *>(right) != 0 && grid[i][j].animal->getFoodLevel() > 30) {
			
			if ((j - 2 >= 0 && j - 2 < maxY) && (i >= 0 && i < maxX) && grid[i][j - 2].animal == NULL) {
	...
	if ((j + 1 >= 0 && j + 1 < maxY) && (i >= 0 && i < maxX) && grid[i][j + 1].animal != NULL && pathFound != true) {
		...
		// Checks to see if its a type of wolf 
		if (dynamic_cast<Wolf *>(down) != 0 && grid[i][j].animal->getFoodLevel() > 30) {
	...
	if ((j >= 0 && j < maxY) && (i - 1 >= 0 && i - 1 < maxX) && grid[i - 1][j].animal != NULL && pathFound != true) {
		...
		// Checks to see if its a type of wolf 
		if (dynamic_cast<Wolf *>(left) != 0 && grid[i][j].animal->getFoodLevel() > 30) {
	...
	if (being_chased == true && pathFound == true) {
		...
		grid[movedX][movedY].SetInnerSquare(sf::Color::Blue);
		grid[movedX][movedY].SetOuterSquare(sf::Color::Blue);
...
void Rabbit::animalEats(int x, int y, SquareYard **& grid) {

	// Rabbit able to eat since it did not evade
	if (grid[x][y].animal->getAction() == false) {

		int currentPlantLevel = grid[x][y].grass->getFoodLevel();

		// Rabbit will only eat however much food the plant has 
...
void Rabbit::setFoodLevel(int change) {
	this->foodLevel += change;
}

void Rabbit::spawnAnimal(int currX, int currY, int prevX, int prevY, SquareYard **& grid, int maxX, int maxY) {
	...
	// Checks the conditions to see if the Rabbit can spawn a baby 
	// Rabbit will be able to reproduce. Since it did not evade 
	if (grid[currX][currY].animal->getAction() == false) {
		...
		if (wolfPresent == false && grid[currX][currY].animal->getProb() == probTarget && grid[prevX][prevY].animal == NULL && grid[currX][currY].animal->getAge() >= 10 && grid[currX][currY].animal->getFoodLevel() >= 40) {
			...
			grid[prevX][prevY].animal = new Rabbit(10, 1, 1, false, false, 50);
			grid[prevX][prevY].SetInnerSquare(sf::Color::Blue);
			grid[prevX][prevY].SetOuterSquare(sf::Color::Blue);
		}
...
bool Rabbit::huntAction(int, int, int, int, SquareYard **&) {
	// Do Nothing 
	return false;
}